...
async def get_db() -> aiosqlite.Connection:
    """Return the shared aiosqlite connection (created once, reused)."""
    global _db
    if _db is None:
...
async def init_db():
    ...
    await db.execute(CREATE_SUMMARIES)
...
async def get_cached_summary(dedup_key: str) -> str | None:
    """Return cached summary for a job, or None if not cached."""
    db = await get_db()
    cursor = await db.execute(
        "SELECT summary FROM summaries WHERE dedup_key = ?", (dedup_key,)
    )
    row = await cursor.fetchone()
    return row[0] if row else None


async def cache_summary(dedup_key: str, summary: str):
    """Store a summary in the cache."""
    db = await get_db()
    await db.execute(
        "INSERT OR REPLACE INTO summaries (dedup_key, summary) VALUES (?, ?)",
        (dedup_key, summary),
    )
    await db.commit()
